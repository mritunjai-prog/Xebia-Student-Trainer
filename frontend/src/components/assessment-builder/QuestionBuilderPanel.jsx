...
export const QuestionBuilderPanel = ({ questions, setQuestions, config, isDesktopConfigOpen, setIsDesktopConfigOpen }) => {
  ...
  const parseWorksheetToJson = (worksheet) => {
    const headers = [];
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      if (Object.values(rowData).some((value) => value !== '' && value != null)) {
        data.push(rowData);
      }
    });
    return data;
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (!worksheet) {
          toast.add('No worksheet found in the Excel file.', 'error');
          return;
        }
  ...
  const handleDownloadTemplate = async () => {
    ...
    link.href = url;
    link.download = 'Assessment_Template_Complete.xlsx';
    link.click();
