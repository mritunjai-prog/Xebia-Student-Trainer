...
export const BatchDetail = () => {
  ...
  const cachedBatch = useMemo(() => batches.find((b) => b.id === id), [batches, id]);

  useEffect(() => {
    let active = true;
    setDetailLoading(true);
    setDetailError('');
    if (cachedBatch) setBatch(cachedBatch);
    ...
    batchApi.get(id)
      ...
      .catch((error) => {
        if (active) setDetailError(error.message || 'Failed to load batch details.');
      })
      .finally(() => {
  ...
  const handleToggleStatus = async () => {
    try {
      const updated = await editBatch(batch.id, batch.name, batch.course, batch.students || [], batch.icon, batch.status === 'active' ? 'inactive' : 'active');
      setBatch(updated);
  ...
  const handleSaveStudents = async () => {
    try {
      const updated = await editBatch(batch.id, batch.name, batch.course, tempStudentIds, batch.icon, batch.status);
      setBatch(updated);
  ...
  const handleSaveEdit = async (e) => {
    ...
    try {
      const updated = await editBatch(batch.id, editName.trim(), editCourse.trim(), batch.students || [], finalIcon, editStatus);
      setBatch(updated);
  ...
  if (detailLoading && !batch) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-neutral-500">
        <h2 className="text-2xl font-bold mb-4 text-neutral-800 dark:text-neutral-200">Loading Batch...</h2>
      </div>
    );
  }

  if (detailError && !batch) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-neutral-500 gap-4">
        <h2 className="text-2xl font-bold text-neutral-800 dark:text-neutral-200">Could Not Load Batch</h2>
