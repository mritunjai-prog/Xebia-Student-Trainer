...
export const StudentAssessments = () => {
  ...
  const studentAssessments = assessments.filter((a) => a.status === 'published');
  ...
  const mappedAssessments = studentAssessments.map((a) => {
    ...
    const completedSubs = subs.filter(s => s.status === 'submitted' || s.status === 'evaluated');
    ...
    return {
      ...
      studentSubmission: completedSubs[completedSubs.length - 1] || subs.find(s => s.status === 'in_progress'),
      maxAttempts
    };
  });
    ...
    <div className="space-y-6 w-full">
      ...
      {studentCoreError && (
        <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/50 text-red-700 dark:text-red-300 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-sm font-semibold">
          <span>{studentCoreError}</span>
          <button
            onClick={() => refreshStudentCoreData().catch((error) => console.error(error))}
      ...
      {!studentCoreLoading && filteredAssessments.length === 0 ? (
      ...
      ) : !studentCoreLoading && viewMode === 'table' ? (
