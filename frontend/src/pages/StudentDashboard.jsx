...
export const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    dashboardApi.student()
      ...
      .catch((error) => {
        if (mounted) {
          setDashboardError(error?.message || 'Student dashboard metrics could not be loaded.');
        }
      })
      .finally(() => {
  ...
  const assignedAssessments = assessments.filter((a) =>
    a.status === 'published' && ((a.batches || []).length === 0 || studentBatchIds.length === 0 || a.batches.some((bId) => studentBatchIds.includes(bId)))
  );
  ...
  const currentRank = dashboardData?.leaderboardRank ?? (rankObj ? rankObj.rank : leaderboard.length);
  ...
  // Recent scores
  const recentScores = dashboardData?.recentScores?.length ? dashboardData.recentScores.map((score) => ({
    id: studentSubmissions.find((submission) => submission.assessmentId === score.id)?.id || score.id,
    assessmentId: score.id,
    title: score.title,
    percentage: score.percentage,
    isEvaluated: score.isEvaluated,
    ...
    <div className="space-y-6">
      ...
      {dashboardError && (
        <div className="p-3 rounded-2xl bg-amber-50 dark:bg-amber-950/20 text-amber-700 dark:text-amber-300 border border-amber-100 dark:border-amber-900/40 text-xs font-semibold">
          {dashboardError} Showing locally derived values where available.
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-700 dark:border-neutral-700/60 shadow-sm p-4 md:p-5 rounded-2xl flex items-center gap-4 hover:-translate-y-1 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
          ...
          <div>
            ...
            <h4 className="text-2xl font-display font-extrabold text-neutral-800 dark:text-white mt-0.5 flex items-baseline gap-1.5">
              {dashboardLoading ? '...' : (dashboardData?.activeAssessments ?? activeAssessments.length)} <span className='text-xs font-semibold text-neutral-400'>Available</span>
            </h4>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-700 dark:border-neutral-700/60 shadow-sm p-4 md:p-5 rounded-2xl flex items-center gap-4 hover:-translate-y-1 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
          ...
          <div>
            ...
            <h4 className="text-2xl font-display font-extrabold text-neutral-800 dark:text-white mt-0.5 flex items-baseline gap-1.5">
              {dashboardLoading ? '...' : (dashboardData?.upcomingAssessments ?? upcomingAssessments.length)} <span className='text-xs font-semibold text-neutral-400'>scheduled</span>
            </h4>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-700 dark:border-neutral-700/60 shadow-sm p-4 md:p-5 rounded-2xl flex items-center gap-4 hover:-translate-y-1 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
          ...
          <div>
            ...
            <h4 className="text-2xl font-display font-extrabold text-neutral-800 dark:text-white mt-0.5 flex items-baseline gap-1.5">
              {dashboardLoading ? '...' : currentRank} <span className='text-xs font-semibold text-neutral-400'>Overall</span>
            </h4>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-700 dark:border-neutral-700/60 shadow-sm p-4 md:p-5 rounded-2xl flex items-center gap-4 hover:-translate-y-1 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
          ...
          <div>
            ...
            <h4 className="text-2xl font-display font-extrabold text-neutral-800 dark:text-white mt-0.5 flex items-baseline gap-1.5">
              {dashboardLoading ? '...' : (dashboardData?.completedAssessments ?? completedAssessments.length)} <span className='text-xs font-semibold text-neutral-400'>submitted</span>
            </h4>
          </div>
        </div>
