...
export const Reports = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      reportApi.overview(),
      reportApi.batches(),
      reportApi.students(params)
    ])
      .then(([overview, batchList, studentList]) => {
        if (mounted) {
          setOverviewReport(overview);
          setBatchReports(batchList);
          setStudentReports(studentList);
          setReportError('');
        }
      })
      .catch((error) => {
        if (mounted) {
          setReportError(error?.message || 'Reports could not be loaded from the backend.');
        }
      })
      .finally(() => {
  ...
  const averageScore = overviewReport ? Math.round(overviewReport.averageScore) : totalEvaluatedCount > 0 ?
  Math.round(evaluatedSubs.reduce((sum, s) => sum + s.percentage, 0) / totalEvaluatedCount) :
  78;

  const highestScore = overviewReport ? Math.round(overviewReport.highestScore) : totalEvaluatedCount > 0 ?
  Math.max(...evaluatedSubs.map((s) => s.percentage)) :
  100;

  const lowestScore = overviewReport ? Math.round(overviewReport.lowestScore) : totalEvaluatedCount > 0 ?
  ...
  const passPercent = overviewReport ? Math.round(overviewReport.passRate) : totalEvaluatedCount > 0 ?
  ...
  const batchComparisonData = batchReports.length ? batchReports.map((report) => ({
    name: report.name,
    averageScore: Math.round(report.averageScore),
    submissionRate: report.submissionCount,
    passingRate: Math.round(report.passingRate),
  ...
  const backendScoreDistribution = (overviewReport?.scoreDistribution || []).map((bucket, index) => ({
    name: bucket.label,
    value: Number(bucket.count) || 0,
    color: ['#E11D48', '#FF6200', '#01AC9F', '#6C1D5F'][index] || '#6C1D5F'
  }));

  const scoreDistribution = backendScoreDistribution.length ? backendScoreDistribution : [
  ...
  const fallbackFilteredStudents = students.filter((s) => {
  ...
  const filteredStudents = studentReports.length ? studentReports : fallbackFilteredStudents;
    ...
    <div className="space-y-6">
      ...
      {reportError && (
        <div className="p-3 rounded-2xl bg-amber-50 dark:bg-amber-950/20 text-amber-700 dark:text-amber-300 border border-amber-100 dark:border-amber-900/40 text-xs font-semibold">
          {reportError} Showing locally derived report values where available.
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-700 dark:border-neutral-800 p-5 rounded-2xl flex items-center justify-between shadow-md">
          <div>
            ...
            <h4 className="text-2xl font-display font-black text-[#6C1D5F] dark:text-purple-300 mt-1">{reportLoading ? '...' : `${averageScore}%`}</h4>
        ...
        <div className="bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-700 dark:border-neutral-800 p-5 rounded-2xl flex items-center justify-between shadow-md">
          <div>
            ...
            <h4 className="text-2xl font-display font-black text-[#01AC9F] dark:text-emerald-400 mt-1">{reportLoading ? '...' : `${passPercent}%`}</h4>
        ...
        <div className="bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-700 dark:border-neutral-800 p-5 rounded-2xl flex items-center justify-between shadow-md">
          <div>
            ...
            <h4 className="text-2xl font-display font-black text-rose-600 mt-1">{reportLoading ? '...' : `${failPercent}%`}</h4>
        ...
        <div className="bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-700 dark:border-neutral-800 p-5 rounded-2xl flex items-center justify-between shadow-md">
          <div>
            ...
            <div className="flex items-baseline gap-2 mt-1">
              <span className="text-xl font-display font-black text-neutral-800 dark:text-white flex items-center"><ArrowUp className="w-4 h-4 text-emerald-500 shrink-0" />{reportLoading ? '...' : `${highestScore}%`}</span>
              <span className="text-neutral-300">/</span>
              <span className="text-sm font-display font-bold text-neutral-500 dark:text-neutral-400 flex items-center"><ArrowDown className="w-3.5 h-3.5 text-rose-500 shrink-0" />{reportLoading ? '...' : `${lowestScore}%`}</span>
            </div>
          ...
          <div className="overflow-x-auto w-full"><table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800/40">
              ...
              filteredStudents.slice(0, 15).map((stud, idx) => {
                const bName = stud.batchNames?.length
                  ? stud.batchNames.join(', ')
                  : batches.find((b) => (stud.batches || []).includes(b.id))?.name || (stud.batches || []).join(', ') || 'Unassigned';
                const score = stud.averageScore || 80;
                const completed = stud.completedCount ?? stud.assessmentsCompleted ?? 0;
                ...
                return (
                  <tr key={stud.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/20 transition-colors">
                      ...
                      <td className="py-3 text-neutral-500 dark:text-neutral-500 dark:text-neutral-400 font-mono">{completed} exams</td>
