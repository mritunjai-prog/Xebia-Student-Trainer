...
export const BatchManagement = () => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    const finalIcon = getFinalIcon();
    try {
      await editBatch(selectedBatch.id, batchName.trim(), course.trim(), enrolledStudentIds, finalIcon, batchStatus);
  ...
  const handleDeleteClick = async (e, batchId, name) => {
    e.stopPropagation();
  ...
  const handleToggleStatus = async (e, batch) => {
    e.stopPropagation();
    const newStatus = batch.status === 'active' ? 'inactive' : 'active';
    try {
      await editBatch(batch.id, batch.name, batch.course, batch.students || [], batch.icon, newStatus);
    ...
    <div className="space-y-6 w-full relative">
        ...
        {teacherCoreError && (
          <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/50 text-red-700 dark:text-red-300 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-sm font-semibold">
            ...
            <button
              ...
              className="px-3 py-2 rounded-xl bg-red-100 dark:bg-red-900/30 hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors flex items-center justify-center gap-2"
            >
        ...
        {teacherCoreLoading && (
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-10 text-center text-neutral-500 shadow-sm font-bold">
            Loading batches and students...
          </div>
        )}

        {/* Content Display */}
        {!teacherCoreLoading && filteredBatches.length === 0 ? (
