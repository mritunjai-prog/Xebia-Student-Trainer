...
export const Leaderboard = () => {
  ...
  useEffect(() => {
    ...
    leaderboardApi.list()
      ...
      .catch((error) => {
        if (mounted) {
          setLeaderboardError(error?.message || 'Leaderboard could not be loaded from the backend.');
        }
      })
      .finally(() => {
  ...
  const leaderboardEntries = backendEntries.length ? backendEntries : getLeaderboard();
    ...
    <div className="space-y-6">
      ...
      {leaderboardError && (
        <div className="p-3 rounded-2xl bg-amber-50 dark:bg-amber-950/20 text-amber-700 dark:text-amber-300 border border-amber-100 dark:border-amber-900/40 text-xs font-semibold">
          {leaderboardError} Showing locally derived rankings where available.
        </div>
      )}

      {leaderboardLoading && (
        <div className="text-xs font-bold text-neutral-500 dark:text-neutral-400">Loading leaderboard...</div>
      )}
