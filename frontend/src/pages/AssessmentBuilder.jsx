...
export const AssessmentBuilder = () => {
  ...
  const handleSaveAssessment = async (e) => {
    ...
    if (activeTab === 'create') {
      try {
        ...
        toast.add(err.message || 'Failed to create assessment.', 'error');
        return;
      }
    } else if (activeTab === 'edit' && editingAssessmentId) {
      try {
        await editAssessment(editingAssessmentId, assessmentPayload);
        toast.add(`Assessment updated!`, 'success');
      } catch (err) {
        toast.add(err.message || 'Failed to update assessment.', 'error');
        return;
      }
    }
  ...
  const handleDuplicate = async (e, id, label) => {
    ...
    try {
      ...
      toast.add(err.message || 'Failed to duplicate assessment.', 'error');
    }
  };
  ...
  const handleDelete = async (e, id, label) => {
    ...
    if (confirm(`Delete assessment "${label}"? This will delete all associated student submissions!`)) {
      try {
        ...
        toast.add(err.message || 'Failed to delete assessment', 'error');
      }
    }
  };

  const handlePublishNow = async (e, id, label) => {
    e.stopPropagation();
    try {
      ...
      <AnimatePresence mode="wait">
        ...
        <motion.div
          ...
          className="space-y-4">
            ...
            {teacherCoreError && (
              <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/50 text-red-700 dark:text-red-300 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-sm font-semibold">
                <span>{teacherCoreError}</span>
                <button
                  onClick={() => refreshTeacherCoreData().catch((error) => toast.add(error.message, 'error'))}
                  className="px-3 py-2 rounded-xl bg-red-100 dark:bg-red-900/30 hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors"
                >
                  Retry
                </button>
              </div>
            ...
            {teacherCoreLoading && (
              <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-10 text-center text-neutral-500 shadow-sm font-bold">
                Loading assessments...
              </div>
            )}

            {/* Content Display based on ViewMode */}
            {!teacherCoreLoading && filteredAssessments.length === 0 ? (
            ...
            ) : !teacherCoreLoading && viewMode === 'table' ? (
